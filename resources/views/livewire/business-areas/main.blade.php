<div>
        ...
        <div class="flex items-center justify-between">
            <div>
                <nav class="text-sm text-gray-500 dark:text-gray-400 mb-2">
                    ...
                    <span class="ml-2">{{ __('Business Area') }}</span>
                </nav>
                <div class="flex items-baseline gap-3">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Business Area') }}</h1>
            ...
            <div class="flex items-center gap-3">
                <div class="relative">
                    <input
                        ...
                        type="search"
                        placeholder="{{ __('search business area...') }}"
                ...
                <div class="relative">
                    <a href="{{ route('business-areas.export') }}" class="inline-flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 rounded-md">
    ...
    <div class="mb-6">
        <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('business-areas.create') }}" class="flex items-center justify-center w-12 h-12 bg-black text-white rounded-lg">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-800/50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors" wire:click="sort('business_name')">
                        {{ __('Business Name') }}
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($businessAreas as $area)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $area->package_name }}</td>
                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $area->state_name }}</td>
                        <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{{ $area->business_name }}</td>
                        <td class="px-4 py-3 text-sm">
                            <span class="inline-flex items-center px-2.5 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 rounded-md">{{ $area->code }}</span>
                        </td>
                        <td class="px-4 py-3 text-sm">
                        ...
                        <td class="px-4 py-3 text-sm">
                            <div class="flex gap-2 justify-center">
                                ...
                                <a href="{{ route('business-areas.edit', $area) }}" class="inline-flex items-center px-2.5 py-1.5 text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-all duration-200" title="Edit Business Area">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                    </svg>
                                </a>
                                <button wire:click="delete({{ $area->id }})" wire:confirm="Are you sure you want to delete this business area? This action cannot be undone." class="inline-flex items-center px-2.5 py-1.5 text-rose-600 dark:text-rose-400 hover:text-rose-800 dark:hover:text-rose-300 transition-all duration-200" title="Delete Business Area">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-12">
                            ...
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ __('No business areas found') }}</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
